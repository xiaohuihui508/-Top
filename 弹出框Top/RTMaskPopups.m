...
@interface RTMaskPopups () <UIGestureRecognizerDelegate>
...
- (void)presentInView:(nullable UIView *)superview
           ...
           completion:(void (^)(RTMaskPopups * _Nonnull))completion {
             ...
             usingSpringWithDamping:_dampingRatio
              initialSpringVelocity:0.2
                         ...
                         animations:^{
                             ...
                             self.popupView.center = [self finishingPoint];
...
- (void)animation:(BOOL)finished completion:(void (^)(RTMaskPopups * _Nonnull))completion {
    if (!finished) return;
    ...
    [UIView animateWithDuration:animated ? _animateDuration : 0
                     ...
                     animations:^{
                         ...
                         if (completion) {
                             ...
                             if (self.delegate && [self.delegate respondsToSelector:@selector(snailQuickMaskPopupsDidDismiss:)]) {
                                 [self.delegate snailQuickMaskPopupsDidDismiss:self];
                             }
                         }
                     }];
}
